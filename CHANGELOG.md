...
Not a soundness guarantee — a conservative, structural approximation with named, honest limits. See `GAPS.md` for the full list, including `broker.summarize()` not currently being serialized against `broker.call()` under concurrent dispatch (a real, documented, unfixed race with a stated workaround).
