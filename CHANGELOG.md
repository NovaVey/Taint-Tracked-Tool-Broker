...
- **Live coverage badge** (Codecov): `vitest.config.ts` now also emits an `lcov` report, uploaded by CI's `coverage` job via `codecov/codecov-action` to back the README's `codecov` badge. Non-blocking (`fail_ci_if_error` defaults to false) — a Codecov outage or upload hiccup never fails this repo's own CI; `vitest.config.ts`'s thresholds remain the actual enforcement gate.
