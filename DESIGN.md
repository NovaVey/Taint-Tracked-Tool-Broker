...
Third: `call()`'s `args` object is handed to the policy function, the approval channel, `execute()`, and the audit sink. If those all shared one live, caller-owned reference, a tool that mutates its own args in place (an ordinary pattern — e.g. appending a delivery receipt) — or any code racing the approval-wait window — could make what a human approved, what actually ran, and what got audited silently diverge. `call()` now snapshots `args` once (`structuredClone`, best-effort) for the `ToolCall` used by policy/approval/audit, and hands `execute()` an independent second snapshot, so nothing any consumer does to its own copy is visible to any other. `ApprovalChannel.requestApproval` also now receives the full `REQUIRE_APPROVAL` decision (not just its reason string), so an implementation can bind its response to `decision.approvalToken` instead of trusting that whatever comes back corresponds to this particular request.
