...
Every large/sensitive tool result that needs condensing costs one extra LLM round trip through `summarize()` — that cost is structural and this library has no way to reduce it. Fingerprint scanning is no longer a flat ongoing cost, either: `InMemoryTaintRegistry` now indexes fuzzy lookups (LSH-banded simhash buckets + a shingle inverted index, `src/taint/registry.ts`) instead of scanning every stored record on every outgoing call, and accepts an optional `maxEntries` (FIFO eviction, oldest-registered first) for long-running sessions that need a memory bound. Neither is free: the index adds bookkeeping on every `register()`, and `maxEntries` is opt-in (unbounded by default) rather than a retention policy this library picks for you — an integrator running a genuinely long-lived session still has to choose a bound and accept that eviction only affects Layer 2 attribution/tightening precision, never the Layer 0 watermark itself (see `registry.ts`'s file header). Named here as an operational concern with better tooling now available, not one this library can make disappear.
