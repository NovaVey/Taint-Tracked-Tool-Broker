...
| `npm run example:irreversible-sink-policy` | A custom `PolicyFn` reading `TaintContext.sinkIrreversible` (GAPS.md #32) to upgrade `ALLOW_WITH_WARNING` to `REQUIRE_APPROVAL` for an irreversible `finance:purchase` sink — and *not* for a reversible `write:fs` sink at the identical scope. |
...
| `npm run example:rbac-policy` | A custom `PolicyFn` reading `TaintContext.principal` (GAPS.md #34) to consult a network-backed RBAC service before letting a gated call through — a bounded inner timeout and a small TTL cache around the lookup, both failing closed to `BLOCK` (never an implicit `ALLOW`) on a timeout, a thrown error, or no principal bound at all. |
