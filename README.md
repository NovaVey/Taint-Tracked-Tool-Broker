...
### The trust lattice and scope watermark
...
### Sink classification and the default policy
...
- **`write:agent-memory`** names a write to the agent's own durable, cross-session memory (a vector store, a memory file, a re-injected scratchpad) distinctly from an ordinary `write:fs`/`irreversible:other` — same `MUTATE` gating, but documenting *why* this write matters: it's the mechanism by which content read this session becomes context in a future one. Classifying the write correctly does nothing for the read side, though — a tool that later reads that memory back still needs its own `isSource: true`, or content re-enters with zero taint regardless of how the write was declared. See GAPS.md #12 and the `agent-memory-cross-session-laundering` corpus case (a true, asserted known gap).
...
### Policy-visible labeling axes — for your own `PolicyFn`; `defaultPolicy` never reads any of these

- **`ToolExecutor.sourceClass`** is a free-form, integrator-defined origin-type label (`'internal-mcp'`, `'public-web'`, `'user-pasted'`, ...) copied onto `ProvenanceTag.sourceClass` every time a source raises the watermark, and surfaced as `TaintContext.sourceClasses` — the distinct classes contributing to the current scope, deduplicated. `TaintLevel` orders trust *degree*; this expresses *why* something is untrusted, which the lattice deliberately collapses. See GAPS.md #28 and `npm run example:source-class-policy`.
- **`SinkCapabilities.irreversible`** is a second, orthogonal axis alongside sink class: class ranks *kind* of side effect (`EXEC` > `EXFIL` > `MUTATE`), not *how bad undoing it would be* — an irreversible `finance:purchase` and a fully reversible `write:fs` scratch write land on the identical policy-matrix cell today. Declare it per sink and read it as `TaintContext.sinkIrreversible` from your own `PolicyFn`. See GAPS.md #32 and `npm run example:irreversible-sink-policy`.
- **`BrokerOptions.principal`** closes a different gap than either axis above: neither answers WHO is asking, only what happened and how tainted the scope is. `createBroker({ principal })` binds an opaque, uninterpreted value (a user id, a role set, a signed JWT — whatever your own boundary asserts) to the broker instance for its whole lifetime, copied through verbatim as `TaintContext.principal` on every real gated/administrative event. This is **not verification** — this library performs no authentication, signature check, or revocation check on it, and it is deliberately kept separate from `sessionId` above rather than repurposing that label. Delegation-chain verification is explicitly out of scope for this library. See GAPS.md #34 and `npm run example:rbac-policy`.

### Layer 2 attribution and the quarantine path
...
- **`ToolExecutor.extractText`** closes a real Layer 2 blind spot: the default text coercion for a source's result only knows how to pass a string through or `JSON.stringify()` anything else, so an image/audio/binary-returning source (a screenshot, a scanned page) registers an unmatchable blob. Declare `extractText(result)` (OCR output, alt-text, a transcript) and the registry gets something an actual future match can find instead — Layer 0's watermark raise is completely unaffected either way. See GAPS.md #33 and `npm run example:screenshot-extract-text`.
...
### Optional structural gates and adoption modes
...
- **`createBroker({ enforcement: 'observe' })`** is a standard adoption-ramp mode (CSP report-only, a WAF's detection mode) for measuring what `'enforce'` (the default) would have gated, against real traffic, before turning enforcement on: `policy()` still runs and every `AuditEvent` is populated identically, but a `BLOCK`/`REQUIRE_APPROVAL`/`QUARANTINE_AND_RETRY` verdict no longer prevents the call — it executes anyway, audited truthfully (`AuditEvent.enforcement: 'observe'`, and `formatAuditTrail()` marks the overridden ones `[OBSERVE MODE: NOT ENFORCED]`). Refuses to construct without a real `auditSink` (`ObserveModeRequiresAuditSinkError`) — a broker that never gates and has nowhere to record what it would have gated is strictly worse than the silent default no-op sink (see [Observability](#observability)). Plan-freeze and `allowedOutboundHosts` are hard structural boundaries independent of `policy()` and remain fully enforced regardless of this setting. See GAPS.md #31.
- **`createBroker({ policyTimeoutMs })`** bounds a network-dependent `PolicyFn` (GAPS.md #35): both real `policy()` call sites run while the broker's own per-instance lock is held, so an unbounded `await` there — a custom `PolicyFn` calling out to an RBAC service that hangs, say — blocks the *entire* broker, not just one caller. Unset (the default), calling `policy()` is byte-for-byte unchanged: no timer, no catch, a throw/rejection propagates exactly as before. Configured, a hang past the timeout *and* a throw/rejection both resolve to the same fail-closed `BLOCK`, mirroring the existing no-`approvalChannel` fail-closed precedent (GAPS.md #20). See `npm run example:rbac-policy` for a worked network-backed `PolicyFn` using this.

### Moving state across a process boundary
...
- **`createTaintEnvelope(value, taint)`** is the same idea at a narrower grain: not a whole broker's state, but one specific value's taint provenance (scope level, matched fingerprint records, a human-readable summary) — for handing that one value across a boundary where the live registry isn't reachable at all (a downstream service, a database row, a human-review UI). One-way, like the audit log, not something this library restores a broker from.

### The audit trail

- **Every gated decision reaches `BrokerOptions.auditSink` as an `AuditEvent`.** See [Observability](#observability) below for the default silent no-op sink, the `JSON.stringify()` hazard on a fuzzy-matched event, the `redactAuditArgs` hook, and the rendering/aggregation utilities.
...
### Core model, injection defense, and adoption modes
...
| `npm run example:openai-tool-loop` | A *raw* OpenAI Chat Completions API-style tool loop (`tools`/`tool_choice`, a manual `while` loop feeding results back as `role:'tool'` messages) — distinct from `example:openai-agents` below, which wires the newer, higher-level Agents SDK instead. |
...
Each wires `broker.wrap()` behind one framework's own tool-definition shape:

| Script | Framework |
|---|---|
| `npm run example:langchain` | LangChain.js — `tool()`/`Runnable.invoke()`. |
| `npm run example:vercel-ai` | Vercel AI SDK — `tool()`/`execute()`. |
| `npm run example:openai-agents` | OpenAI Agents SDK — `tool()`/`execute()`, exercising the `REQUIRE_APPROVAL` path via `createDeferredApprovalChannel()`. |
| `npm run example:mastra` | Mastra — `createTool({ execute: ({ context }) => ... })`. |
| `npm run example:genkit` | Google Genkit — `ai.defineTool()`'s handler shape. |
| `npm run example:agent-sdk` | The (Claude) Agent SDK's in-process `tool()`/`createSdkMcpServer()` helper — a third, distinct Anthropic integration shape alongside `example:tool-loop` (the raw Messages API) and `example:mcp-sdk` (a real network MCP server). |
| `npm run example:llamaindex-ts` | LlamaIndex.TS — `FunctionTool.from()`/`tool()`. |
| `npm run example:semantic-kernel-js` | Semantic Kernel JS — `KernelFunction.from()`/plugin shape. |

None of these depend on the real `langchain`/`ai`/`@openai/agents`/`mastra`/`genkit`/`llamaindex`/`@microsoft/semantic-kernel` packages — each uses a small structural stand-in for that framework's real tool-definition shape, since the integration point (a `name`/`description`/schema object with an async execute function) is what matters, not fidelity to a fast-moving package's exact current types. Every framework's real dispatch loop calls that function the same way once `broker.wrap()` has interposed it. `example:mcp-sdk` above is the one exception: it depends on the real `@modelcontextprotocol/sdk` (a devDependency, not a runtime dependency of this library) to confirm the stand-in pattern used by `example:mcp` actually holds against the genuine SDK's current shapes.
...
### Optional axes and extension points

Each demonstrates one policy-visible axis or attribution hook from [Core model](#core-model) above, in a custom `PolicyFn` or tool declaration:

| Script | Axis / hook |
|---|---|
| `npm run example:source-class-policy` | `TaintContext.sourceClasses` (GAPS.md #28) — downgrades `REQUIRE_APPROVAL` when every contributing source is a reviewed internal MCP server, and *not* when a public-web source is also in scope. |
| `npm run example:irreversible-sink-policy` | `TaintContext.sinkIrreversible` (GAPS.md #32) — upgrades `ALLOW_WITH_WARNING` to `REQUIRE_APPROVAL` for an irreversible `finance:purchase` sink, and *not* for a reversible `write:fs` sink at the identical scope. |
| `npm run example:rbac-policy` | `TaintContext.principal` (GAPS.md #34) — consults a network-backed RBAC service before letting a gated call through, with a bounded inner timeout and a small TTL cache, both failing closed to `BLOCK` (never an implicit `ALLOW`) on a timeout, a thrown error, or no principal bound at all. |
| `npm run example:screenshot-extract-text` | `ToolExecutor.extractText` (GAPS.md #33) — a screenshot source's OCR text registers into Layer 2 and later triggers an exact-match attribution; no OCR text found skips registration cleanly (watermark still raises); no `extractText` declared falls back to an unmatchable stringified blob. |
