...
Writes, network egress, state changes, purchases, anything irreversible — declare the real `SinkCapability` (`exec:shell` / `exec:code` / `write:fs` / `write:external-account` / `finance:purchase` / `write:agent-memory` / `irreversible:other` / `net:outbound` / `net:email` / `net:api-call` / `net:post-message`). The trap here isn't forgetting an obviously-privileged tool's capability — it's a tool that *looks* read-only but has a side effect one layer down (a "read-only" API call that also logs the query to a third-party analytics backend is an EXFIL sink whether or not its primary purpose is a read). `capabilities: []` is a claim, not a default to reach for when unsure — see example 3 below.

A write to the agent's own durable, cross-session memory (a vector store, a memory file, a scratchpad an orchestration layer re-injects into a future turn or session) is `write:agent-memory`, not `write:fs`/`irreversible:other` — same `MUTATE` gating either way, but naming it distinctly documents its cross-session significance for the next person reading the declaration (GAPS.md #12). **Correctly classifying the write is only half the story**: a separate tool that later READS this same memory store back needs its OWN `isSource: true` declaration (question 1 above) — otherwise content written here re-enters the model's context with zero taint the moment it's read, regardless of how carefully the write itself was classified. See GAPS.md #12's own paragraph on this and the `agent-memory-cross-session-laundering` corpus case for the concrete demonstration.
