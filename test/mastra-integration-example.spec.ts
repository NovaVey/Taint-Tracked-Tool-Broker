...
// examples/mastra-integration.ts (like every file under examples/) is
...
// `npm run example:mastra` does (`tsx` in a subprocess) is the only way to
...
const exampleScriptPath = fileURLToPath(
  new URL('../examples/mastra-integration.ts', import.meta.url),
);

describe('examples/mastra-integration.ts', () => {
  it('routes createTool()/execute({ context }) through broker.wrap() and actually gates the shell_exec call', async () => {
    ...
    // wrapAsMastraTool's createTool()-shaped execute is actually being
    ...
    // An unrecognized tool id must propagate as a real error and never be
    ...
    expect(stdout).toContain(
      'unrecognized tool id correctly propagated as a real error, not mislabeled as an allowed call: no such tool: shell_exce',
    );
